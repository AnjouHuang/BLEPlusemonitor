...
#define DEVICE_NAME                         "Pluse Monitor"                            /**< Name of device. Will be included in the advertising data. */
            ...
            while (app_uart_put('\n') == NRF_ERROR_BUSY);
				...
				switch(p_evt->params.rx_data.p_data[0])
				{
					...
					case 'C':
						...
						file_manage_state = 1;
//						app_timer_start(m_timer_speed, APP_TIMER_TICKS(10),NULL);
						
						break;
					
					case 'D':
						printf("stop file transfer \r\n");
						file_manage_state = false;
//						app_timer_stop(m_timer_speed);
//						m_len_sent = 0;
//						m_cnt_7ms = 0;
						break;
...
static void heart_rate_meas_timeout_handler(void * p_context)
{
    ...
    cnt++;
    err_code = ble_hrs_heart_rate_measurement_send(&m_hrs, adc_val);
    if ((err_code != NRF_SUCCESS) &&
        (err_code != NRF_ERROR_INVALID_STATE) &&
        (err_code != NRF_ERROR_RESOURCES) &&
        (err_code != NRF_ERROR_BUSY) &&
...
static void advertising_init(void)
{
    ...
    init.advdata.name_type               = BLE_ADVDATA_FULL_NAME;//全名广播
    init.advdata.include_appearance      = true;
    init.advdata.flags                   = BLE_GAP_ADV_FLAGS_LE_ONLY_GENERAL_DISC_MODE;//LE普通发现模式
...
void throughput_timer_handler(void * p_context)
{
	ret_code_t err_code;
	uint16_t length;
	m_cnt_7ms++;
  ...
  length = m_ble_nus_max_data_len;
   ...
   printf("time: %d *20ms == bytes send: %d Bytes == avg speed: %d B/s \r\n",m_cnt_7ms,m_len_sent,(m_len_sent * 1000)/(m_cnt_7ms*10));
}
...
uint32_t data_send(uint8_t data[],uint16_t length)
{
		ret_code_t err_code;
