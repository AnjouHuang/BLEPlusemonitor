...
uint8_t Tx_Buffer[10] = {"helloworld"};
...
void spi_flash_test(void){
                ...
                nrf_delay_ms(100);						
					      SpiFlash_Write_Page(Tx_Buffer,0x00,10);//写入5个字节数据
								for(i=0; i<10;i++)printf(" %c",(uint8_t)Tx_Buffer[i]);//串口打印从FLASH读出的数据
			          for(i=0; i<10;i++)Rx_Buffer[i] = 0; //清零Flash_WR_Buf
					      SpiFlash_Read(Rx_Buffer,0x00,10);      //读出5个字节数据
					      printf("Read data = ");
					      for(i=0; i<10;i++)printf(" %c",(uint8_t)Rx_Buffer[i]);//串口打印从FLASH读出的数据
							  printf("\r\n");  //回车换行
